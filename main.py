...
def main(page: ft.Page):
    ...
    # Şirket logosunun yolu (Eğer varsa)
    logo_yolu = "ozdemir_logo.png"
    if os.path.exists(logo_yolu):
        page.window_icon = logo_yolu
    ...
    # Android Uyumlu Dosya Seçici Ayarı
    aktif_kategori = [None]
    ...
    logo_resmi = ft.Image(src=logo_yolu, width=120, height=50, fit=ft.ImageFit.CONTAIN) if os.path.exists(logo_yolu) else ft.Container()
    baslik_alani = ft.Row([
        logo_resmi,
    ...
    def orjinal_excele_kaydet(e):
        # Android İndirilenler klasörü yolları
        sablon_yolu = "/sdcard/Download/INSPECTION REPORT.xlsx"
        cikti_yolu = f"/sdcard/Download/OZDEMIRTEKS_TEFTIS_RAPORU_PO_{po_input.value}.xlsx"
        
        if not os.path.exists(sablon_yolu):
            bilgi_notu.value = "Hata: Cihazın 'Download' klasöründe 'INSPECTION REPORT.xlsx' şablonu bulunamadı!"
            bilgi_notu.color = ft.Colors.RED_700
            page.update()
            return

        try:
            bilgi_notu.value = "Rapor hazırlanıyor, lütfen bekleyin..."
            ...
            for anahtar, hedef_kelime, hucre in resim_haritasi:
                ...
                if resim_yolu and os.path.exists(resim_yolu):
                    ...
                    if gercek_sekme_adi:
                        ...
                        # Hücre taşmalarını önlemek için otomatik orantılı küçültme
    ...
    resim_listesi = ft.Column([
        ...
        resim_secici_butonu("Paketli Arka Görünüm", "paketli_back"),
